  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (errorCode === 'auth/unauthorized-domain') {
        const punycode = "xn--espaodosoculos-jjb.com.br";
        const currentDomain = window.location.hostname;
        
        alert(`ERRO DE CONFIGURAÇÃO NO SEU FIREBASE:
        
O domínio "${currentDomain}" não está autorizado no seu painel do Firebase.

Siga estes 2 passos agora:
1. Vá no Firebase Console > Authentication > Settings > Authorized Domains.
2. Adicione estes DOIS endereços exatamente assim:
   - ${currentDomain}
   - ${punycode}

Sem isso, o Google bloqueia o acesso por segurança.`);
      } else {
        alert(`Erro ao fazer login (${errorCode}). Verifique sua conexão ou se as chaves no arquivo .env estão corretas.`);
      }
    }
  };
